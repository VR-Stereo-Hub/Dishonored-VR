# Compile and exercise the production query-helper profiler and trampoline; never launch the game.
$ErrorActionPreference = 'Stop'
$repo = Split-Path -Parent $PSScriptRoot
$eyeOut = Join-Path $repo 'build\query-wait-test'
...
try {
    ...
    & "$eyeVc\bin\Hostx64\x86\cl.exe" /nologo /std:c++20 /EHsc /W3 "/I$PSScriptRoot\query-wait-stubs" "/I$repo\src" /Fe:query_wait_test.exe (Join-Path $PSScriptRoot 'query-wait-tests.cpp') /link /INCREMENTAL:NO
