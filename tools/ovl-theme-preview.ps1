...
param([switch]$Advanced, [switch]$DebugTier, [int]$Height = 0, [ValidateRange(640,2048)][int]$Size = 1254, [switch]$Tooltip, [switch]$Bottom, [float]$TextScale = 0)
...
try {
    ...
    $mode = if ($DebugTier) { 'debug' } elseif ($Advanced) { 'advanced' } else { 'basic' }
