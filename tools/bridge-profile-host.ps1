...
$out = Join-Path $repo "build\bridge-profile-tests"
...
Push-Location $out
try {
    & "$root\bin\Hostx64\x86\cl.exe" /nologo /EHsc /W4 /std:c++17 /I (Join-Path $repo "src") /Fe:bridge-profile-tests.exe (Join-Path $PSScriptRoot "bridge-profile-tests.cpp")
    if ($LASTEXITCODE -ne 0) { throw "bridge-profile compilation failed." }
    .\bridge-profile-tests.exe
