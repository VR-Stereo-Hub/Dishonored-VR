...
$dll    = Join-Path $outDir "dvr_xrsim32.dll"

if (-not (Test-Path $dll)) {
    throw "dvr_xrsim32.dll not found in $outDir - run .\tools\build.ps1$(if ($Release) {' -Release'}) first."
}
...
$escaped  = $dll.Replace('\', '\\')
$json     = '{"file_format_version":"1.0.0","runtime":{"name":"dvr-xrsim","library_path":"' + $escaped + '"}}'
