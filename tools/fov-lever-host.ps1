...
$out = Join-Path $repo "build\fov-lever-tests"
...
Push-Location $out
try {
    & "$root\bin\Hostx64\x86\cl.exe" /nologo /EHsc /W4 /std:c++17 /Fe:fov-lever-tests.exe (Join-Path $PSScriptRoot "fov-lever-tests.cpp")
