# Validate policy and native D3D9Ex without loading the mod or game.
param()
$ErrorActionPreference = 'Stop'
$repo = Split-Path -Parent $PSScriptRoot
$eyeOut = Join-Path $repo 'build\nonblocking-present-test'
...
try {
    ...
    & "$eyeVc\bin\Hostx64\x86\cl.exe" /nologo /std:c++20 /EHsc /W3 /I. /I (Join-Path $repo 'src') /I (Join-Path $repo 'build\generated') /Fe:nonblocking_present_test.exe (Join-Path $PSScriptRoot 'nonblocking-present-tests.cpp') /link user32.lib
    if ($LASTEXITCODE -ne 0) { throw 'Default-profile test compilation failed.' }
    & .\nonblocking_present_test.exe
