...
if ($out -notmatch "runtime:\s*dvr-xrsim") {
    Fail "xr_hello32 did not report runtime 'dvr-xrsim'. XR_RUNTIME_JSON did not take (elevated shell? bad manifest path? 64-bit dll?)."
}
...
Write-Host ""
Write-Host "SELFTEST PASS: dvr-xrsim ran $($s.frame) frames, reached $($s.sessionState), 0 errors." -ForegroundColor Green
exit 0
