# Timed desktop Present A/B/A. Never launches the game or starts a recorder.
# Commands are live only; the installed default-off INI remains unchanged.
[CmdletBinding()]
param(
...
try {
 ...
 Command 'desktopnonblocking off'
 Command 'mark desktop_baseline_before'
 PauseGame 30
 $changed=$true
 Command 'desktopnonblocking on'
 Command 'mark desktop_nonblocking_start'
 PauseGame 40
 Command 'desktopnonblocking off'
 $changed=$false
 Command 'mark desktop_baseline_after'
 PauseGame 30
 Command 'mark desktop_test_complete'
 ...
 if ($changed -and $game -and !$game.HasExited) {
  try { Command 'desktopnonblocking off' } catch {
