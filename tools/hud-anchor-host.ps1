# Compile and run the HUD anchor math (VR-117) on the host. Never launches the game.
$ErrorActionPreference = "Stop"
$repo = Split-Path -Parent $PSScriptRoot
$out = Join-Path $repo "build\hud-anchor-tests"
...
Push-Location $out
try {
    & "$root\bin\Hostx64\x86\cl.exe" /nologo /EHsc /W4 /std:c++17 /I (Join-Path $repo "src") /Fe:hud-anchor-tests.exe (Join-Path $PSScriptRoot "hud-anchor-tests.cpp")
    if ($LASTEXITCODE -ne 0) { throw "hud-anchor compilation failed." }
    .\hud-anchor-tests.exe
