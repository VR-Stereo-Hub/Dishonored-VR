...
#   2. dishonoredvr.log mtime is advancing        - the wedge signature: the log stops
...
#
# Usage:
#   .\tools\soak.ps1 -Minutes 10 -Arm "vrstereo on"
#   .\tools\soak.ps1 -Minutes 5  -Arm "reentry srdev on; vrstereo on"
#   .\tools\soak.ps1 -Minutes 10 -Arm none          # vanilla flat
#   .\tools\soak.ps1 -Attach -Minutes 3 -Arm "vrcam on"
#   .\tools\soak.ps1 -Boot map -Map Ghetto.bsm -Minutes 10 -Arm "vrstereo on"
#
...
#   9 preflight refused (the game is already running)
[CmdletBinding()]
param(
...
function Invoke-Preflight {
    try {
        & (Join-Path $PSScriptRoot "launch-game.ps1") -PreflightOnly -Force:$Force |
...
if ($Attach) {
    ...
    if ($Boot -eq "map") {
        ...
        & (Join-Path $PSScriptRoot "launch-game.ps1") -Force:$Force |
            ForEach-Object { Write-Step $_ }
    }
}
...
for ($i = 0; $i -lt $GameplayTimeoutSeconds; $i++) {
    ...
    if (($Boot -eq "key" -or $Boot -eq "map") -and -not $keyTried -and $i -ge $keyAt) {
        ...
        if (Test-Path $gk) {
            ...
            & $gk -Key Space -Repeat 3 -NoFocus:$NoFocus | ForEach-Object { Write-Step $_ }
        } else {
...
if (-not $gameplay) {
    ...
    Show-Tail $log 20 "dishonoredvr.log"
    exit 5
}
...
if ($armed) {
    ...
    try {
        & (Join-Path $PSScriptRoot "game-batch.ps1") -NoFocus:$NoFocus -Delay 3 $Arm |
            ForEach-Object { Write-Step $_ }
    } catch {
...
while ((Get-Date) -lt $deadline) {
    ...
    if (Test-Path $log) {
        ...
        if ($age -gt 20) {
            Write-Step "FAIL after ${elapsed}s: dishonoredvr.log has not advanced for $([int]$age)s - WEDGED"
            Show-Tail $trace 40 "pacetrace.log"
            Show-Tail $log 20 "dishonoredvr.log"
            Stop-GameIfAsked
            exit 2
