...
static uint32_t g_sdProgressInside = 0;
...
static void SceneDrawBeat()
{
    ...
    Log("reentry: beat draws/s=%.0f 2nd/s=%.0f presents/s=%.0f call2=%u us (max %u) skips foreign=%lu state=%lu "
        ...
        "progressInsideDraw=%u (otherwise-valid stereo ticks the old return-baseline would reject) | "
        ...
        g_sdSumCall2Us / 1000.0 / n, g_sdCall2MaxUs / 1000.0, g_sdProgressInside, g_sdSumOutsideUs / 1000.0 / n,
...
static SdDecision SceneDrawDecide(uint32_t callerRet)
{
    ...
    // Present-stall guard (liveness only): count from the previous draw ENTRY,
    // including presents during that draw. Pulses still bypass this guard.
    if (!g_sdPresentProgress.advanced && !d.pulse) { ++g_sdSkipStall; d.why = "no present since the previous draw"; return d; }
