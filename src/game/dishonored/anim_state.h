...
struct Snapshot {
    ...
    bool valid = false, game = false, cameraAction = false, showArms = false;
};
void tick();
...
bool native_full_arms(); // geometry visibility, independent of native pose
bool native_draw(); // blend reached native identity: release controller placement
