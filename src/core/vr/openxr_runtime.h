#pragma once
// core/vr/openxr_runtime.h - the VR runtime layer, adopted from the BioShock
// trilogy VR mod in 41.0 (namespace renamed, otherwise kept as close to the
// original as the D3D9 host allows). It owns the OpenXR instance, session,
// swapchains, frame pacing (with the pace thread and the detach/feed guards),
// the head/hand pose funnel, the laser/aim-dot/HUD quads and the eye-tag
// bookkeeping for AlternateEye and SequentialReentry stereo. Fail-soft: no
// runtime, no headset, or any XR error leaves the game running flat.
//
// The D3D9 host (Dishonored) differs from BioShock in two places only: the
// mod PROVIDES the D3D11 device (set_device_provider; created on the adapter
// the runtime names) instead of borrowing the game's, and the frame to show
// arrives as a texture from the stereo seam (on_present_end(frame)) instead
// of being read off a DXGI swapchain. Swapchains follow that texture's size.
//
// Threading: init_instance() runs once from Direct3DCreate9 (off the loader
// lock); everything else runs on the thread that calls Present - the thread
// that owns every runtime call. The pace thread only runs xrWaitFrame by
// request.
...
// Present-hook tail: copy `frame` (the texture the stereo method produced
// for this present; null = nothing to show, the frame still ends) into the
// swapchain the eye tags select, build the layers, xrEndFrame. The swapchains
// are created on the first frame at its size and rebuilt when it changes; the
// texture must be R8G8B8A8 (CopyResource needs the same typeless family as
// the swapchain, see swapchain_format()).
void on_present_end(ID3D11Texture2D* frame);

// --- 41.0: the D3D9 host's seams -------------------------------------------
// The mod provides the D3D11 device. Called once per bring-up with the adapter
// LUID the runtime requires (xrGetD3D11GraphicsRequirementsKHR); the provider
// creates or returns its device on THAT adapter (the 39.3 fix). The runtime
// AddRefs what it gets. Null = bring-up fails soft and retries.
using DeviceProviderFn = ID3D11Device* (*)(const _LUID* wanted);
void set_device_provider(DeviceProviderFn fn);
// [VR] Runtime=auto|native|steamvr and [VR] XrRuntimeJson, set by the config
// BEFORE init_instance. The json sets XR_RUNTIME_JSON for this process when
...
// The runtime's name once an instance exists ("none" before).
const char* runtime_name();
// Inter-eye separation from the located views (meters); false until located.
bool eye_separation_m(float* out);
// A haptic pulse for one hand (0 left, 1 right); any thread, applied on the
// present thread inside the next action sync.
void haptic(int hand, float amp, float durSec);
// Tear the session (and the instance) down; present thread. Used at PreExit.
