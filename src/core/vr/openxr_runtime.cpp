// core/vr/openxr_runtime.cpp - the VR runtime layer (see the header). Adopted
// from the BioShock trilogy VR mod in 41.0: the text below is that file with
// the namespace renamed and the D3D9-host seams marked "41.0 (Dishonored)".
// Its session numbers (s28, s34, s54...) refer to that project's log, which
...
#define XRLOG(...) DVR_LOG(::dvr::log::Cat::openxr, ::dvr::log::Level::Info, __VA_ARGS__)
...
ID3D11Device* g_device = nullptr;          // the mod's device (provider), AddRef'd
ID3D11DeviceContext* g_context = nullptr;  // immediate context, AddRef'd

// 41.0 (Dishonored): the D3D9 host has no DXGI swapchain to borrow a device
// from, so the mod provides one - created on the adapter the runtime names
// (core/gfx/d3d11_device.cpp), asked for once per bring-up.
DeviceProviderFn g_deviceProvider = nullptr;
char g_runtimeMode[16] = "auto";          // [VR] Runtime=auto|native|steamvr
char g_runtimeJson[2 * MAX_PATH] = "";    // [VR] XrRuntimeJson
// The frame-texture geometry the swapchains are built for: taken from the
// first frame the stereo method submits, rebuilt when it changes.
uint32_t g_wantW = 0, g_wantH = 0, g_wantFmt = 0;

// %LOCALAPPDATA%\DishonoredVR as a wide path (the mod's paths module is narrow).
bool data_dir_w(wchar_t* out /*MAX_PATH*/) {
    const char* d = dvr::paths::data_dir();
    if (!d || !d[0]) return false;
    return MultiByteToWideChar(CP_ACP, 0, d, -1, out, MAX_PATH) > 0;
}
...
XrPosef parallel_eye_tag(const XrPosef& l, const XrPosef& r, int eye, float ipdMm) {
    ...
    float off[3];
    dvr::xrmath::quat_rotate(q[0], q[1], q[2], q[3], axis, off);
    XrPosef out{};
...
int sr_pop_eye() {
    ...
    if (head - tail > 2) {
        ...
        XRLOG("xr: sr tag ring skewed (depth %u) - cleared", head - tail);
        return 0;
    }
...
XrResult feed_submit_cycle(XrSession s, const XrFrameState& fs) {
    ...
    if (XR_SUCCEEDED(r)) {
        if (g_feedCycles.fetch_add(1, std::memory_order_relaxed) == 0)
            XRLOG("xr: pace feed live - re-submitting the last %s layer while the "
    ...
    while (g_paceRun.load(std::memory_order_relaxed)) {
        ...
        if (ms > 1000) {
            ...
            if (t0 - lastStallLogMs > 5000) {
                lastStallLogMs = t0;
                XRLOG("xr: %s blocked %u ms on the pace thread (state "
...
bool pace_thread_start() {
    ...
    XRLOG("xr: pace thread started - xrWaitFrame no longer runs on the present "
...
bool pace_should_skip(XrSessionState state, bool everFocused, uint64_t now) {
    ...
    if (g_unfocusedSinceMs == 0) {
        ...
        XRLOG("xr: pacing SKIPPED while %s (headset idle) - the game keeps "
...
// The trace writes to its OWN file with its OWN handle. XRLOG serialises on a
...
void trace_thread_start() {
    ...
    if (g_traceThread)
        XRLOG("xr: pace trace started - 1 Hz from its OWN thread, so it keeps "
...
bool detach_skip_decision(uint64_t now, bool focused) {
    ...
    if (focused) {
        if (g_detachedNow) {
            g_detachedNow = false;
            XRLOG("xr: ATTACHED - session FOCUSED again, full-rate pacing resumes "
    ...
    if (!g_detachedNow) {
        ...
        XRLOG("xr: DETACHED (session %s) - the present thread now makes NO blocking "
...
void composite_hud() {
    // 41.0 (Dishonored): nothing captures the HUD yet (hud_stub.h), and the
    // flat window is the game's own D3D9 backbuffer. No-op.
}
...
void teardown_session(const char* why) {
    ...
    if (g_paceOutstanding) {
        if (WaitForSingleObject(g_paceDone, 200) == WAIT_OBJECT_0) {
            ...
            if (g_teardownPending != why)
                XRLOG("xr: teardown (%s) DEFERRED - a wait is still in flight on "
    ...
    XRLOG("xr: session teardown (%s)", why);
...
void create_laser(int64_t format) {
    ...
    if (FAILED(g_device->CreateTexture2D(&td, &init, &g_laserDot))) {
        XRLOG("xr: laser dot texture creation failed - aim laser unavailable");
        g_laserDot = nullptr;
        return;
    }
    ...
    if (XR_FAILED(r)) {
        XRLOG("xr: laser swapchain creation failed: %s", res_str(r));
        g_laserSwapchain = XR_NULL_HANDLE;
        destroy_laser();
        return;
    }
    ...
    if (XR_FAILED(xrEnumerateSwapchainImages(
        ...
        XRLOG("xr: laser swapchain image enumeration failed");
        destroy_laser();
        return;
    }
...
bool create_swapchains(uint32_t width, uint32_t height, uint32_t format) {
    ...
    if (!pick) {
        XRLOG("xr: no R8G8B8A8 swapchain format offered - cannot copy backbuffer");
        return false;
    }
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (XR_FAILED(r)) {
            XRLOG("xr: xrCreateSwapchain failed: %s", res_str(r));
            g_swapchains[i] = XR_NULL_HANDLE;
            destroy_swapchains();
            return false;
        }
        ...
        if (XR_FAILED(r)) {
            XRLOG("xr: xrEnumerateSwapchainImages failed: %s", res_str(r));
            destroy_swapchains();
            return false;
        }
    }
    ...
    g_backbufferFmt = format; // for the same-size resize guard
    XRLOG("xr: swapchain pair %ux%u format %lld (%u images each)", g_swapW, g_swapH,
...
void try_bring_up() {
    ...
    if (r == XR_ERROR_FORM_FACTOR_UNAVAILABLE) {
        if (!g_loggedNoHmd) {
            XRLOG("xr: no headset connected (runtime '%s') - will keep retrying quietly",
                    g_runtimeName);
            g_loggedNoHmd = true;
        }
    ...
    if (XR_FAILED(r)) {
        XRLOG("xr: xrGetSystem failed: %s", res_str(r));
        g_nextRetryMs = GetTickCount64() + 5000;
        return;
    }
    g_loggedNoHmd = false;
    ...
    // recommended_eye_size() to annotate their resolution pickers.
    {
        ...
        if (XR_SUCCEEDED(vr) && nViews >= 1) {
            ...
            g_recommendedEyeH.store(vcv[0].recommendedImageRectHeight,
                                    std::memory_order_relaxed);
            XRLOG("xr: view config recommends %ux%u per eye (max %ux%u, %u views)",
                    vcv[0].recommendedImageRectWidth, vcv[0].recommendedImageRectHeight,
                    vcv[0].maxImageRectWidth, vcv[0].maxImageRectHeight, nViews);
        } else {
            XRLOG("xr: xrEnumerateViewConfigurationViews failed: %s (informational only)",
                    res_str(vr));
        }
    }
    ...
    if (!getReqs || XR_FAILED(r = getReqs(g_instance, g_system, &reqs))) {
        XRLOG("xr: xrGetD3D11GraphicsRequirementsKHR failed: %s",
    ...
    // 41.0 (Dishonored): the mod's own D3D11 device, on the adapter the runtime
    // asked for (the 39.3 fix: a device on the wrong GPU succeeds at every call
    // and shows nothing). AddRef'd so teardown's Release balances.
    ID3D11Device* dev = g_deviceProvider ? g_deviceProvider(&reqs.adapterLuid) : nullptr;
    if (!dev) {
        XRLOG("xr: no D3D11 device from the mod (provider %s) - retrying in 5 s",
              g_deviceProvider ? "failed" : "not set");
        g_nextRetryMs = GetTickCount64() + 5000;
        return;
    }
    g_device = dev;
    ...
    XRLOG("xr: mod D3D11 device feature level 0x%X, runtime min 0x%X",
    ...
    if (XR_FAILED(r)) {
        XRLOG("xr: xrCreateSession failed: %s", res_str(r));
    ...
    if (XR_FAILED(r)) {
        XRLOG("xr: xrCreateReferenceSpace failed: %s", res_str(r));
    ...
    if (XR_FAILED(r)) {
        XRLOG("xr: xrCreateReferenceSpace(VIEW) failed: %s", res_str(r));
        teardown_session("view space failed");
        return;
    }

    // 41.0 (Dishonored): the swapchains are built when the first frame texture
    // arrives (on_present_end) - the stereo method decides the eye size.
    ...
    XRLOG("xr: session created on the mod's D3D11 device - waiting for READY");
}
...
void pump_events() {
    ...
    while (xrPollEvent(g_instance, &ev) == XR_SUCCESS) {
        if (ev.type == XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED) {
            ...
            g_state = sc->state;
            XRLOG("xr: session state %s", state_str(g_state));
            switch (g_state) {
                case XR_SESSION_STATE_READY: {
                    ...
                    if (XR_FAILED(r)) {
                        XRLOG("xr: xrBeginSession failed: %s", res_str(r));
                        teardown_session("begin failed");
                    } else {
                        g_sessionBegun = true;
                        XRLOG("xr: session running - game is now paced by the headset");
                        trace_thread_start();
                    }
                    break;
                }
                case XR_SESSION_STATE_FOCUSED:
                    ...
                    if (g_unfocusedSinceMs != 0)
                        XRLOG("xr: FOCUSED again after %llu ms unfocused - full "
                ...
                case XR_SESSION_STATE_STOPPING:
                    if (g_sessionBegun) {
                        ...
                        g_unfocusedSinceMs = 0;
                        g_paceSkips.store(0, std::memory_order_relaxed);
                        XRLOG("xr: session stopped (headset idle?) - waiting for READY again");
                    }
                    break;
            ...
            // mirrors.
            XRLOG("xr: interaction profile changed (runtime rebound controllers)");
        }
...
//
// Mode override: [VR] Runtime=auto|native|steamvr in dishonored_vr.ini
// (set_runtime_mode, called by the config before init_instance).

// Root data dir (%LOCALAPPDATA%\DishonoredVR), NOT the per-game subdir - the
...
bool xr_root_dir(wchar_t* out /*MAX_PATH*/) {
    ...
    if (FAILED(SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, local)))
        return false;
    swprintf_s(out, MAX_PATH, L"%s\\DishonoredVR", local);
    return true;
}

// Returns the mode string ("auto"/"native"/"steamvr") the config set.
void xr_mode_read(char* mode, size_t n) {
    strcpy_s(mode, n, g_runtimeMode);
    if (_stricmp(mode, "auto") != 0) XRLOG("xr: runtime mode '%s' ([VR] Runtime)", mode);
}

// The shim ships beside d3d9.dll (both live next to the game exe).
bool shim_dll_path(wchar_t* out /*MAX_PATH*/) {
    ...
    *slash = 0;
    swprintf_s(out, MAX_PATH, L"%s\\dvr_steamvr32.dll", path);
...
bool shim_write_manifest(const wchar_t* shimDll, wchar_t* manifestOut /*MAX_PATH*/) {
    ...
    swprintf_s(manifestOut, MAX_PATH, L"%s\\dvr_steamvr32.json", dir);
    ...
    fprintf(f,
            ...
            "    \"name\": \"DishonoredVR SteamVR shim (OpenVR)\",\n"
            "    \"library_path\": \"%s\"\n  }\n}\n",
            escaped);
    fclose(f);
    return true;
...
void log_active_runtime_expectation() {
    ...
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Khronos\\OpenXR\\1", 0,
        ...
        XRLOG("xr: no 32-bit ActiveRuntime key registered - expecting to use "
    ...
    if (rs != ERROR_SUCCESS || type != REG_SZ || !val[0]) {
        XRLOG("xr: 32-bit ActiveRuntime value missing - expecting to use the "
...
XrResult try_create_instance(const char* label, bool quietExplainer) {
    ...
    if (XR_FAILED(r)) {
        XRLOG("xr: [%s] no 32-bit OpenXR runtime reachable (%d)", label,
                static_cast<int>(r));
        return r;
    }
...
void init_instance() {
    ...
    // [VR] XrRuntimeJson: a manifest the loader must use for THIS launch (the
    ...
    if (g_runtimeJson[0]) {
        ...
        if (GetEnvironmentVariableA("XR_RUNTIME_JSON", cur, sizeof(cur)) == 0) {
            SetEnvironmentVariableA("XR_RUNTIME_JSON", g_runtimeJson);
            XRLOG("xr: XR_RUNTIME_JSON set from [VR] XrRuntimeJson: %s", g_runtimeJson);
        } else {
    ...
    if (XR_FAILED(r)) {
        if (_stricmp(mode, "native") == 0 || !mayFallBack)
            XRLOG("xr: VR disabled, game runs flat");
        return;
    }
...
void on_present_begin() {
            ...
            if (g_paceOutstanding &&
                ...
                if (kind == kPaceReqWait && XR_SUCCEEDED(r)) {
                    ...
                    if (!loggedFeedFail.exchange(true))
                        XRLOG("xr: feed cycle failed: %s (backing off 1 s between "
    ...
    if (g_srPairOpen) {
        ...
        XRLOG("xr: pair hold outlived %u ms with no right-eye present - "
    ...
    if (g_resizePending.load(std::memory_order_acquire) && !g_frameOpen && !g_srPairOpen) {
        g_resizePending.store(false, std::memory_order_relaxed);
        XRLOG("xr: performing the queued XR swapchain rebuild (no frame open)");
        destroy_swapchains();
    }
    ...
    if (g_swapchains[0] == XR_NULL_HANDLE && g_wantW && g_wantH) {
        g_hfovDeg.store(0.0f, std::memory_order_relaxed);
        if (!create_swapchains(g_wantW, g_wantH, g_wantFmt)) {
            g_projectionReady.store(false, std::memory_order_relaxed);
            return;
        }
    }
    ...
    if (g_frameOpen) {
        ...
        g_frameOpen = false;
        g_srPairOpen = false;
        XRLOG("xr: closed a leaked open frame before waiting (pair aborted?)");
    }
    ...
    if (g_paceOffThread.load(std::memory_order_relaxed) && pace_thread_start()) {
        ...
        if (waitMs > 1000) {
            ...
            if (waitStart - lastStallLogMs > 5000) {
                ...
                XRLOG("xr: xrWaitFrame blocked %u ms INLINE on the present "
    ...
    if (XR_FAILED(r)) {
        XRLOG("xr: xrBeginFrame failed: %s", res_str(r));
    ...
    if (g_viewsValid && g_viewLogLeft.load(std::memory_order_relaxed) > 0) {
        ...
        XRLOG("xr: viewlog L pos %.4f %.4f %.4f quat %.4f %.4f %.4f %.4f fov "
                "%.2f/%.2f/%.2f/%.2f",
        ...
        XRLOG("xr: viewlog R pos %.4f %.4f %.4f quat %.4f %.4f %.4f %.4f fov "
                "%.2f/%.2f/%.2f/%.2f",
        ...
        XRLOG("xr: viewlog derived: eyeSep %.4f m (d %.4f %.4f %.4f) | cant %.4f deg "
    ...
    if (!ready && g_cameraMode.load(std::memory_order_relaxed)) {
        ...
        if (now - g_lastProjBlockedLogMs > 5000) {
            ...
            XRLOG("xr: camera mode requested but projection not ready "
...
uint32_t build_laser_from(const LaserSnapshot& ls, XrCompositionLayerQuad* quads,
    ...
    if (ls.muzzle) {
        ...
        dvr::xrmath::xr_local_trim_quat(ls.modelPitch * kDegToRad,
                                        ls.modelYaw * kDegToRad,
                                        ls.modelRoll * kDegToRad, trim);
        dvr::xrmath::quat_mul(quat, trim, q2);
        dvr::xrmath::quat_rotate(q2[0], q2[1], q2[2], q2[3], d0, d);
    } else {
        dvr::xrmath::xr_local_trim_quat(ls.pitchTrim * kDegToRad,
                                        ls.yawTrim * kDegToRad, 0.0f, trim);
        dvr::xrmath::quat_mul(quat, trim, q2);
        dvr::xrmath::quat_rotate(q2[0], q2[1], q2[2], q2[3], fwd, d);
    }
    ...
    if (built && !g_loggedFirstLaser.exchange(true))
        XRLOG("xr: aim laser live (%u dot layers, hand %c)", built, hand ? 'R' : 'L');
    return built;
}
...
uint32_t build_hand_ref_quad(XrCompositionLayerQuad* quad) {
    ...
    if (!g_loggedFirstHandQuad.exchange(true))
        XRLOG("xr: hand ref quad live (grip %c at xr %.3f %.3f %.3f, %.2f m from the "
...
float xr_quat_yaw_deg(float qx, float qy, float qz, float qw) {
    ...
    dvr::xrmath::quat_rotate(qx, qy, qz, qw, fwd, f);
...
void on_present_end(ID3D11Texture2D* frame) {
    ...
    if (!g_frameOpen) {
        ...
        if (g_session != XR_NULL_HANDLE && now - g_lastIdleLogMs >= 5000) {
            ...
            XRLOG("xr: SUBMISSION IDLE (reason=%s | state=%s everFocused=%d "
    ...
    if (g_cineEnabled.load(std::memory_order_relaxed) && projectionMode) {
        ...
        bool fovMm = dvr::hud::fov_mismatch();
        bool screenOnly = dvr::hud::screen_only(); // hack/loading/FMV screens
        ...
        if (wantCine != active) {
            if (++g_cineStreak >= kCineHysteresis) {
                ...
                XRLOG("xr: cinematic quad %s (strict=%d stale=%d fovMismatch=%d "
                        "screenOnly=%d)",
        ...
        if (active) {
            ...
            if (dvr::hud::fov_watch(&t, nullptr, &age, 500) && t > 0.05f) {
                hfovDeg = 2.0f * atanf(t) * 57.29578f;
                hfovSrc = 3; // "live"
                if (!g_loggedLiveClaim.exchange(true))
                    XRLOG("xr: claim substituted from the live WORLD lens "
    ...
    // 41.0 (Dishonored): the swapchains follow the frame texture the stereo
    // method hands in - built on the first one, rebuilt (through the queued
    // on_resize path) when its size moves.
    if (frame) {
        D3D11_TEXTURE2D_DESC fd{};
        frame->GetDesc(&fd);
        if (g_swapchains[0] == XR_NULL_HANDLE) {
            g_wantW = fd.Width; g_wantH = fd.Height; g_wantFmt = fd.Format;
            g_hfovDeg.store(0.0f, std::memory_order_relaxed);
            if (!create_swapchains(g_wantW, g_wantH, g_wantFmt))
                g_projectionReady.store(false, std::memory_order_relaxed);
        } else if (fd.Width != g_swapW || fd.Height != g_swapH) {
            g_wantW = fd.Width; g_wantH = fd.Height; g_wantFmt = fd.Format;
            on_resize(fd.Width, fd.Height, fd.Format);
        }
    }

    if (g_frameState.shouldRender && g_swapchains[0] != XR_NULL_HANDLE) {
        ID3D11Texture2D* backbuffer = frame;
        if (backbuffer) {
            backbuffer->AddRef(); // the Release sites below balance this
            ...
            if (XR_SUCCEEDED(acqRes)) {
                ...
                if (projectionMode) {
                    ...
                    if (tanClaimH != g_auditTanH.load(std::memory_order_relaxed) ||
                        ...
                        XRLOG("xr: fovaudit submit tanH=%.6f tanV=%.6f (hfov %.2f deg, "
                    ...
                    if (!g_loggedFirstProjection.exchange(true))
                        XRLOG("xr: first projection-layer frame (claimed hfov %.1f deg)",
                                hfovDeg);
                    if (stereo && !g_loggedFirstStereo.exchange(true))
                        XRLOG("xr: alternate-eye stereo live (both eyes hold offset images)");
                } else {
    ...
    if (layerCount && ++g_framesSubmitted == 1)
        XRLOG("xr: first frame submitted to the headset (%ux%u quad)", g_swapW, g_swapH);
...
void on_resize(unsigned width, unsigned height, unsigned format) {
    ...
    if (sameSize) {
        XRLOG("xr: same-size frame texture (%ux%u fmt %u) - XR swapchains kept",
                width, height, format);
        return;
    }
    ...
    XRLOG("xr: frame texture size changed (%ux%u fmt %u) - XR swapchain rebuild QUEUED for a safe "
...
void set_camera_mode(bool on) {
    ...
    if (was != on)
        XRLOG("xr: camera mode %s (request; drive engages when the session "
...
void set_enabled(bool on) {
    ...
    if (was != on) XRLOG("xr: VR %s (preset/programmatic)", on ? "ENABLED" : "disabled");
}
...
void set_alternate_eye(bool on) {
    ...
    XRLOG("xr: AlternateEye stereo %s (per-eye images on alternate frames; the "
...
void set_eye_tag_rendered(bool on) {
    ...
    if (was != on)
        XRLOG("xr: eye tags = %s (rendered = the parallel pair the game drew: "
...
void set_hand_ref_quad(bool on, int hand, float sizeDeg) {
    ...
    if (was != on)
        XRLOG("xr: hand ref quad = %s (hand %c, %.1f deg) - parked at the located "
...
void set_edge_snapshot(bool on) {
    ...
    if (was != on) XRLOG("xr: edge-telemetry snapshot %s", on ? "ARMED" : "off");
    if (!on) {
        std::lock_guard<std::mutex> lk(g_edgeSnapMutex);
...
void arm_view_log(int frames) {
    ...
    XRLOG("xr: view log ARMED for %d frames (located per-eye pose + fov, plus "
            "inter-eye deltas)",
            frames);
}
...
void set_pace_sync(bool on) {
    ...
    if (was != on)
        XRLOG("xr: pair-rate sync %s (adapter/preset; `vrpace sync` for the "
...
void set_pose_lag(int lag) {
    ...
    if (was != lag)
        XRLOG("xr: pose attribution lag %d -> %d generation(s) (0=fresh, "
...
void set_spike_trace(bool on) {
    ...
    if (was != on) {
        ...
        XRLOG("xr: spike trace %s (pair interval > 2x period -> per-phase "
...
void handle_pace_command(const char* args) {
    ...
    if (strcmp(verb, "on") == 0) {
        ...
        XRLOG("xr: pace guard ON (unfocused session skips the blocking wait)");
    } else if (strcmp(verb, "off") == 0) {
        g_paceGuard.store(false, std::memory_order_relaxed);
        XRLOG("xr: pace guard OFF (pre-M8 behavior: every present waits, the "
        ...
        XRLOG("xr: xrWaitFrame %s (session 28: off-thread is what lets us keep "
        ...
        if (sscanf_s(rest, "%u", &ms) == 1 && ms >= 100 && ms <= 10000) {
            ...
            XRLOG("xr: unfocused keepalive every %u ms", ms);
        } else {
            XRLOG("xr: usage: vrpace keepalive <100..10000 ms>  (current %u)",
        ...
        XRLOG("xr: detached pacing %s - while the session is not FOCUSED the "
        ...
        bool on = strncmp(rest, "off", 3) != 0;
        set_pace_feed(on);
        XRLOG("xr: (feed %s via vrpace; cycles so far %u, %u layered)",
        ...
        if (strncmp(rest, "on", 2) == 0) {
            ...
            XRLOG("xr: pair-rate sync ON (target = the runtime's "
            ...
            XRLOG("xr: pair-rate sync OFF (pairs open at the game's own present "
            ...
            g_paceSync.store(true, std::memory_order_relaxed);
            XRLOG("xr: pair-rate sync ON at a COMMANDED %u Hz (overrides the "
                    "runtime period; `vrpace sync on` returns to the period)",
                    hz);
        } else {
            XRLOG("xr: pair-rate sync %s (hz override %u) | delayed %u pairs, "
        ...
        if (strncmp(rest, "on", 2) == 0) {
            ...
            XRLOG("xr: spike trace %s | %u spikes captured (threshold 2x period, "
        ...
        XRLOG("xr: simulated idle %s%s", on ? "ON" : "off",
        ...
        XRLOG("xr: detach %s (keepalive every %u ms) | detachedNow=%d | episodes %u "
                ...
                g_detachSkips.load(std::memory_order_relaxed),
                g_detachKeepalives.load(std::memory_order_relaxed));
        XRLOG("xr: feed %s | cycles %u (%u layered) | snapshot %s "
...
void handle_mirror_command(const char* args) {
    if (strncmp(args, "on", 2) == 0) {
        ...
        XRLOG("xr: desktop mirror ON (window pinned to the LEFT eye under stereo)");
    } else if (strncmp(args, "off", 3) == 0) {
        g_mirror.store(false, std::memory_order_relaxed);
        XRLOG("xr: desktop mirror OFF (pre-M8 behavior: the window alternates "
                "eyes under stereo)");
    } else {
        XRLOG("xr: mirror %s | holds %u blits %u (vrmirror on|off|status)",
...
void set_pace_detach(bool on) {
    ...
    XRLOG("xr: detached pacing %s by the game adapter (an unfocused session "
...
void set_pace_feed(bool on) {
    ...
    XRLOG("xr: pace feed %s by the game adapter - while the session is not "
...
void set_cine_drive(CineDrive mode) {
    ...
    if (g_cineDrive.exchange(v, std::memory_order_relaxed) != v)
        XRLOG("xr: cinematic drive = %s", cine_drive_name(mode));
}
...
void handle_cine_command(const char* args) {
    if (strncmp(args, "drive off", 9) == 0) {
        ...
        dvr::hud::set_bars_hidden(true);
    } else if (strncmp(args, "bars show", 9) == 0) {
        dvr::hud::set_bars_hidden(false);
    } else if (strncmp(args, "effects verts", 13) == 0) {
        unsigned n = 0;
        if (sscanf_s(args + 13, "%u", &n) == 1 && n >= 3)
            dvr::hud::set_effect_max_verts(n);
        else
            XRLOG("xr: usage: vrcine effects verts <n>  (current %u)",
                    dvr::hud::effect_max_verts());
    } else if (strncmp(args, "effects frame", 13) == 0) {
        dvr::hud::set_effects_in_frame(true);
    } else if (strncmp(args, "effects panel", 13) == 0) {
        dvr::hud::set_effects_in_frame(false);
    } else if (strncmp(args, "postfx cine on", 14) == 0) {
        dvr::hud::set_postfx_cine_size(true);
    } else if (strncmp(args, "postfx cine off", 15) == 0) {
        dvr::hud::set_postfx_cine_size(false);
    } else if (strncmp(args, "postfx rt", 9) == 0) {
        dvr::hud::set_postfx_rt_only(true);
    } else if (strncmp(args, "postfx size", 11) == 0) {
        dvr::hud::set_postfx_rt_only(false);
    } else if (strncmp(args, "restorert on", 12) == 0) {
        dvr::hud::set_restore_rt(true);
    } else if (strncmp(args, "restorert off", 13) == 0) {
        dvr::hud::set_restore_rt(false);
    } else if (strncmp(args, "subs panel", 10) == 0) {
        dvr::hud::set_cine_subs_in_frame(false);
    } else if (strncmp(args, "subs frame", 10) == 0) {
        dvr::hud::set_cine_subs_in_frame(true);
    } else if (strncmp(args, "bars verts", 10) == 0) {
        unsigned n = 0;
        if (sscanf_s(args + 10, "%u", &n) == 1 && n >= 3)
            dvr::hud::set_bar_verts(n);
        else
            XRLOG("xr: usage: vrcine bars verts <n>  (current %u)", dvr::hud::bar_verts());
    } else if (strncmp(args, "dumparm off", 11) == 0) {
        dvr::hud::set_dump_on_edge(0, 2);
        XRLOG("xr: edge dump DISARMED");
        ...
        dvr::hud::set_dump_on_edge(1, n);
        XRLOG("xr: edge dump armed on the next BAR-DRAW rising edge (%d windows)", n);
    } else if (strncmp(args, "dumparm screen", 14) == 0) {
        int n = 2;
        sscanf_s(args + 14, "%d", &n);
        dvr::hud::set_dump_on_edge(2, n);
        XRLOG("xr: edge dump armed on the next SCREEN-ONLY rising edge (%d windows)", n);
    } else if (strncmp(args, "dumparm letterbox", 17) == 0) {
        int n = 2;
        sscanf_s(args + 17, "%d", &n);
        dvr::hud::set_dump_on_edge(3, n);
        XRLOG("xr: edge dump armed on the next LETTERBOX pixel-watch rising edge "
                "(%d windows)", n);
        ...
        XRLOG("xr: 'vrcine unsqueeze' is RETIRED - the bars are a gameswf draw over a "
        ...
        XRLOG("xr: cinematic mode STEREO (fov-mismatch scenes keep the projection, "
        ...
        XRLOG("xr: cinematic mode QUAD (fov-mismatch scenes drop to the big screen)");
    } else if (strncmp(args, "on", 2) == 0) {
        g_cineEnabled.store(true, std::memory_order_relaxed);
        XRLOG("xr: cinematic fallback ON (non-gameplay views drop to the quad screen)");
    } else if (strncmp(args, "off", 3) == 0) {
        g_cineEnabled.store(false, std::memory_order_relaxed);
        XRLOG("xr: cinematic fallback OFF (pre-session-22: cutscenes submit as a "
        ...
        bool haveFov = dvr::hud::fov_watch(&t, &tv, &fovAge, 0); // print stale too
        unsigned barsSkipped = 0, barIntervals = 0, barVerts = 0;
        dvr::hud::get_bar_stats(&barsSkipped, &barIntervals, &barVerts);
        unsigned lbTop = 0, lbBot = 0;
        bool lbPix = dvr::hud::letterbox(&lbTop, &lbBot);
        bool barDraw = dvr::hud::bar_draw_active();
        ...
        XRLOG("xr: cine bars=%s (looking for %u verts) | barDraw=%d (skipped %u, "
                "intervals %u, last %u verts) | pixelWatch=%d (top %u bot %u) | sources %s",
                dvr::hud::bars_hidden() ? "HIDDEN" : "shown", dvr::hud::bar_verts(),
        ...
        unsigned strandedTotal = 0;
        for (int i = 0; i < dvr::hud::kRoutePassCount; ++i) strandedTotal += rs.stranded[i];
        XRLOG("xr: cine effects=%s bound=%u verts (inFrame %u, over-bound %u) | "
                "stranded effect=%u total=%u restored=%u (restorert %s) | postFx=%u "
                "(rejected by the bind test %u, square target=%d, rule=%s)",
                dvr::hud::effects_in_frame() ? "IN-FRAME" : "panel",
                dvr::hud::effect_max_verts(), rs.effectsInFrame, rs.effectsRejected,
                rs.stranded[dvr::hud::kRouteEffect], strandedTotal, rs.restored,
                dvr::hud::restore_rt() ? "ON" : "off", rs.postFx,
                rs.postFxRejected, rs.squareTarget ? 1 : 0,
                dvr::hud::postfx_rt_only()
                    ? (dvr::hud::postfx_cine_size() ? "render-target (size-only in cutscenes)"
                                                    : "render-target")
                    : "size-only");
        int dumpEdge = 0, dumpCount = 0;
        dvr::hud::get_dump_on_edge(&dumpEdge, &dumpCount);
        XRLOG("xr: cine %s mode=%s active=%d | enters %u exits %u presents %u | "
                ...
                dvr::hud::fov_mismatch() ? 1 : 0, dvr::hud::screen_only() ? 1 : 0,
...
void set_device_provider(DeviceProviderFn fn) { g_deviceProvider = fn; }

void set_runtime_mode(const char* mode) {
    if (!mode || !mode[0]) return;
    if (_stricmp(mode, "native") == 0 || _stricmp(mode, "steamvr") == 0 ||
        _stricmp(mode, "auto") == 0)
        strcpy_s(g_runtimeMode, mode);
    else
        XRLOG("xr: [VR] Runtime '%s' not recognized (auto|native|steamvr) - using 'auto'", mode);
}

void set_runtime_json(const char* manifestPath) {
    if (manifestPath) strcpy_s(g_runtimeJson, manifestPath);
...
int64_t swapchain_format() { return g_swapFormat; }

const char* runtime_name() { return g_runtimeName; }

bool eye_separation_m(float* out) {
    if (!out || !g_viewsValid) return false;
...
void shutdown(const char* why) {
    ...
    if (g_session != XR_NULL_HANDLE) {
        XRLOG("xr: shutdown (%s) - session teardown deferred, instance kept", why);
        return;
    }
    if (g_instance != XR_NULL_HANDLE) {
        xrDestroyInstance(g_instance);
        g_instance = XR_NULL_HANDLE;
        XRLOG("xr: shutdown (%s) - instance destroyed", why);
    }
}

} // namespace dvr::vr

#else // !DVR_WITH_OPENXR

namespace dvr::vr {

void init_instance() {
    XRLOG("xr: built without OpenXR support - VR disabled");
}
void on_present_begin() {}
void on_present_end(ID3D11Texture2D*) {}
void set_device_provider(DeviceProviderFn) {}
