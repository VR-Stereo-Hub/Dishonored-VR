...
// What the device did between the previous present and this one, and this present's
// arguments (captured at Present entry).
struct PresentActivity {
    uint32_t draws = 0, begins = 0, srts = 0;
    bool srcRect = false, dstRect = false, hwnd = false, dirty = false;
};
PresentActivity present_activity();
