...
bool g_mirrorOff = true, g_submitRefused = false; // Default off on every runtime; explicit settings stay honored.
...
void set_mirror_off(bool on) {
    ...
    DVR_INFO("desktoppresent: DesktopMirrorOff=%d; setting honored on every runtime. OFF freezes desktop updates "
             "while XR capture remains live, guarded fallback still presents", g_mirrorOff);
}
