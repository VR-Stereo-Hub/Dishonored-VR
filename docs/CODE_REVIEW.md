...
| 28 | **The D3D11 device is created on the default adapter** (`D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE...)`) while `xrGetD3D11GraphicsRequirementsKHR` is called and its `adapterLuid` discarded; the game-frame and eye textures use `D3D11_RESOURCE_MISC_SHARED`, which cannot cross adapters and has no keyed mutex. On a two-GPU PC the runtime gets surfaces on the wrong GPU and nothing logs an error | 3224 (`EnsureD3D11`), 3245/4120 (`MISC_SHARED`), 21975 | the author's best explanation of "works only on his PC" (handoff section 7) | **next (D1a)**: port the 39.3 fix - enumerate adapters via `dxgi.dll`, match the LUID, `D3D_DRIVER_TYPE_UNKNOWN` |
| 29 | The two menu-ghost rescues cover `menuOpen && !cursorVis` and `cursorVis && !menuOpen`, so `menuOpen && cursorVis` (the normal windowed state) is covered by neither; one missed close event parks head-look until F9 | menu detector | "F9 fixes it" reports | next (D1a): port the 39.4 fix (drop the `!cursorVis` requirement; the engine stops dispatching view rotation while paused) |
| 30 | The path-3 head fallback is gated on the act of writing the rotator, not on the write having an effect; on a machine where the engine discards the pitch write, head-look is dead AND the rescue is suppressed | `ApplyHeadToViewRotation`, `g_fbPvrSince` | the Quest no-pitch bug | next (D1a): port the 39.2 kept/discarded loop |
| 31 | Calibration records keyed by component pointer; a weapon swap recreates the component and re-probes mid-combat, sometimes with inverted signs | `FpCalibrateTick`, `FpCollect` | "wiggling weapons" | next (D1a): port the 39.0 bank by asset name |
| 32 | Three head-motion paths (head-mouse, native script write, direct fallback) are on at once; the bugs found on the author's last day were in the gates between them | `[Tracking] Enabled`, `[HeadTrack] Native`, viewinject | six hand-offs to get stuck in | roadmap: collapse to one path, only with the game in front of you |
