...
| Does the eye offset point where it should? (S2) | log / status | drive or turn a full circle, then `camera/eyesep:` | the VERDICT line; or `status.json` -> `camera.rowSweepDeg` vs `headSweepDeg`. A row sweep near the head sweep = a real camera basis row; a row sweep near 0 with a large head sweep = the offset is on a fixed world axis. **Both near 0 means you did not turn** - that is the control, not a result |
| Is the projection layer actually reaching the headset? (S2) | log / status | `tail-log.ps1 -Grep "cinematic quad"` | `xr: cinematic quad off (strict=1 stale=0)` and `status.json` -> `aerCineQuad false`. ON in gameplay means the gameplay verdict is false: read `pawn oracle:` and `[game] state:` next, not the eye tags. `vrcine off` is the live A/B |
| Is there a gameplay pawn at all? | log | `pawn oracle:` | `source now ctrl+0x248` = healthy; `source now none` names which of the controller and the event latch is missing. The event latch NEVER fills in this game (ENGINE_NOTES) |
