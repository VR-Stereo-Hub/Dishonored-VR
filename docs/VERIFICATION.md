...
| Do the two pose lanes agree? (the ghosting) | log | `game-cmd.ps1 "vrpace poseaudit on"` in GAMEPLAY under `stereo reentry`, then turn the head | first `xr: poseaudit SEAM CHECK ok` (if it says FAILED the sign calibration is wrong and every delta below is meaningless), then `xr: poseaudit L tag .. R tag .. vs SCRIPT-lane .. -> delta L .. R ..` every 500 ms. `GENERATION GAP 0` and a delta near 0.00 = the lanes agree and pose attribution is NOT the ghosting; a steady nonzero gap names how many locates the tag is off by. `vrpace lag 0\|1\|2` must move it monotonically and one value must null it. L and R must match |
...
| What are the game's own option settings set to? (VR-157) | log | **nothing to run - it reads itself once, a few seconds into gameplay.** F10 Advanced -> "Read them into the log now" re-runs it; `game-cmd.ps1 "gameopts"` is the scripted route | one line per setting: the profile's value through the engine's own `GetProfileSettingValueInt`, and what the renderer holds now through `scale get <key>`. **A disagreement between the two columns is the finding, not an error** - the inis are a mirror and have already been caught lying about light shafts. `NO ANSWER` means the engine refused, NOT that the setting is 0; `MISMATCH - do not believe this row` means the id's own name did not match the PSI table and that row is not evidence. A `REFUSED` line instead of any rows names which of the three lookups failed. `[Diagnostics] GameOptsOnStart=0` turns the automatic read off |
...
| Does the HUD reach the compositor as a quad? (VR-117) | capture + log | `xrsim-run.ps1 -Path tools\xrsim\hud-panel.xrs` from GAMEPLAY | `quadLayers` 0 with `hud off`, `>= 1` with `hud on`, 0 again; the log's `hud/beat ... -> ARMED` with `empty-while-armed=0` (the line says what an empty present would mean); `dump hud` writes the sink (geometry only, R/B swapped) |
| Do the HUD anchors place their quads? (VR-117, VR-120) | capture | `hud-quads.xrs` (poses the sim's left hand first; `hud anchor all <anchor>` moves every row) | `hud_window.json`: quads in `view` space at z -1.30 (the vitals crop at (-0.50, 0.37), the reticle at (0, -0.10)); `hud_hand.json`: quads in `local` space at the posed grip plus the 0.06 m lift; `hud_hand_untracked`: `quadLayers` 0 (no stale quad); `hud_world_a/b.json`: the same LOCAL poses before and after `head rot 0 30 0` |
| Does every element ride its own anchor? (VR-120) | capture + log | `hud-elements.xrs` (poses both hands) | with the preset `quadLayers >= 2` (the vitals and the reticle as separate quads); `hud anchor vitals handL; hud anchor reticle handR` -> `quadLayers 2`, `hud_el_hands.json` two `local` quads at (-0.20, 1.16, -0.40) and (0.20, 1.16, -0.40), 0.22 m wide; `hand r valid off` -> 1; `hud anchor vitals frame` -> 1 with the log's `hud/layout: routed ... left in frame N`; `hud anchor vitals off` -> 1; the log's `hud/layout: sink N = <anchor>/<crop|all>` lines name every sink; host: `tools\hud-route-host.ps1` 20 checks |
| Does the pause menu ride the HUD window? (VR-117) | capture + log | `pause-ride.xrs` from GAMEPLAY | through the pause: `projectionViews` stays 2, `quadLayers >= 1`, the log's `ui/ride: Pause -> RIDING` and `stereo/state: STEREO ... standIn=riding`, the `stereo: beat` line reading L/s=R/s>0 mono/s=0; after 10 s no `menu: stale flag cleared`; on resume `eyeAgeL/R 0` (`projStaleSubmits` climbs through a ride by design, 634 over a 12 s pause, so the sequence no longer asserts it); with `hud menu off` the same Escape gives `projectionViews 0` (the mono screen, the A/B) |
| Does the HUD quad keep its coverage in every alpha mode? (VR-119) | capture + state | `xrsim-run.ps1 -Path tools\xrsim\hud-alpha.xrs` from GAMEPLAY | after each `@shot`, `quadAlphaPct >= 1` in `repair`, `captured` and `mix` (the first quad layer's share of pixels with alpha > 8/255; a mode that collapsed to alpha 0 fails it), `>= 50` with a floor of 0.5 and a half-opaque window backdrop; `dump hud 0` writes `hud_s0_<f>_alpha.png` (grey = alpha): the vitals' dark strokes present in `captured`, faint in `repair`; the log's `draws/blend:` line names the HUD's colour equation and `draws/alpha: forcing ...` says the coverage equation is on |
| Which registers carry the HUD's 2D transform, and what are its elements? (VR-118) | log | `draws on`, `hud regions on`, `draws vsdump`, then `draws regions` | `draws/xform: vs <hash>: position v0 -> o1 = c6*x + c7*y + c8*z + c9*w` once per shader (a shader that does not parse says so and its draws get no rectangle); `draws/regions: ... 0 refused ... HUD draws with a vertex shader N, without 0; SetTransform calls 0`; every bucket `bbox` inside [0,1]; `draws/cluster:` one line per element candidate by frequency (the vitals top-left, the reticle at 0.5,0.5, the prompt right of it); `draws vsdump` writes `<data_dir>\dumps\vs_<hash>_ps_<hash>.txt` (game-derived, never committed) |
| Does the power wheel ride the HUD window? (VR-117) | capture + log | `wheel-ride.xrs` from GAMEPLAY (a held left grip is the wheel) | `projectionViews` stays 2 with the grip held, `quadLayers >= 1`, `ui/surface: context=Wheel ... rides=1`; released: both eyes fresh; with `hud menu Wheel off` the same hold gives `projectionViews 0` (the A/B) |
...
| Did what the game wrote into a texture ever reach the GPU? (VR-15, the black texture bug) | log | `device upload` (also rides with `device census` at the first GAMEPLAY) | one row per texture class - `twin=N (failed F) NO-TWIN=N (refused F) passthrough=N (failed F)` with the first `HRESULT` - then the twin population (`live, N of them have carried NO successful UpdateTexture`), then the surface-bypass counts, then a VERDICT that reads either `NO LOST UPLOAD` (the shadow is not the cause: go to `[Device] Ex=0`, then `stereo arm off`, then no mod) or `N LOST UPLOADS` naming which of the four paths lost them |
| Are the small mips what is black at distance? (VR-15, the leading hypothesis) | headset | `[Device] ShadowFullCopy=1` in the ini (or `device shadowfullcopy on` live), then walk toward a surface that was black | black at distance is a MIP fault, because distance selects the level. With the lever off the log's `device/upload` mip line reads `N unlocks carried a level > 0` against 0 levels pushed; with it on, `N levels pushed with UpdateSurface` climbs and `refused` stays low. **The verdict is what your eyes say about the far surface**, not the counter: the counter only proves the lever ran |
| Is the shadow involved at all? (VR-15, the control) | headset | `[Device] Ex=0` in the ini, relaunch | turns off the 9Ex device, the translation and the shadow together (the capture falls back to the CPU readback, so it will be slower - that is expected and is not the thing being judged). Black at distance SURVIVING `Ex=0` exonerates the shadow completely and sends the hunt to `stereo arm off`, then the game with no mod. Cheapest decisive test; run it first |
| Is the `GetSurfaceLevel` bypass what makes surfaces black? (VR-15) | headset | `device upload` to read the bypass count, then `device shadowsurfaces on` and look | the bypass count is `surfaces handed out .. locked (N of those on a shadowed texture)`; with the lever on, `redirected` climbs and `refused` stays 0. Ships OFF: it is a lever, so it needs a headset verdict before it can become a default, and a bypass count of 0 means the lever is inert and the black surfaces are something else |
