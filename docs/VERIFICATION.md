...
| Do both HUD anchors place their quads? (VR-117) | capture | `hud-quads.xrs` (poses the sim's left hand first) | `hud_window.json`: a quad in `view` space at (0, -0.10, -1.30); `hud_hand.json`: a quad in `local` space within 0.15 m of the posed hand; `hud_hand_untracked`: `quadLayers` 0 (no stale quad); `hud_world_a/b.json`: the same LOCAL pose before and after `head rot 0 30 0` |
