...
| What are the game's own option settings set to? (VR-157) | log | `game-cmd.ps1 "gameopts"` in GAMEPLAY | one line per setting: the profile's value through the engine's own `GetProfileSettingValueInt`, and what the renderer holds now through `scale get <key>`. **A disagreement between the two columns is the finding, not an error** - the inis are a mirror and have already been caught lying about light shafts. `NO ANSWER` means the engine refused, NOT that the setting is 0; `MISMATCH - do not believe this row` means the id's own name did not match the PSI table and that row is not evidence. A `REFUSED` line instead of any rows names which of the three lookups failed. Needs GAMEPLAY: the profile object comes off the player controller |
