...
`VR-Main` is at the VR-68 merge. The tester's verdict on the branch: real fixes
landed, and the residual flicker plus the churn around it leave the mod **worse
to play than at the start of the session**. Do not open a PR for it as it stands.

### Confirmed working, in a headset

* **Render size** honoured end to end (VR-66, merged). 3190x3306 ran; the engine
  has no ceiling - it was asking for a size nobody advertised.
* **Head-turn judder** gone (`[Pace] Lag=2`, VR-65, merged).
* **Weapon judder** gone (`[Hands] PoseLag=2`, VR-68, merged).
* **The large weapon flicker and the double image** gone, and weapon **scale and
  depth are correct** (`PaletteEyeOffset=1` driven by the measured eye with
  `PaletteEyeMeasSign=-1`).

### The residual, and what it now affects

**Hands, weapons AND world geometry disturb together**, 1-2 times a second, left
eye. Orientation-independent. Only the first two are placed by the mod, so
either the world is a second fault or all three share a cause upstream of
placement.
...
| **Absolute position agreement (writer vs c5)** | **already tried 2026-09-03**: fails while WALKING, dropped a third of left tags, invisible on a stationary simulator |
| The held-layer partial pair | counterprediction FAILS - `aborts=0` across 22 summaries while holds ran at 1.33/s |

### THE ACTUAL BLOCKER

**Nothing carries a verified engine VIEW identity from view construction, through
the render thread, to the palette draw.** Every attempt above substituted
something else for that key - a delta, a position, a publication, a matrix - and
each failed differently. `FLICKER_IMPLEMENTATION_PLAN.md` (as revised by review)
names Phase 0: find the engine boundary that constructs/enqueues a render view
and the boundary that consumes it, and carry an identity between them.

**That is a reverse-engineering task, and it is the next session's whole job.**

### Instruments now trustworthy

* `eye_observer.h` + `eye_observer_test.h` - 8 fixtures, run at init and by
  `frame_test.exe`. Its first case is the defect that cost a headset run.
* `perf_ab.cpp` - pair distributions with separate median and tail noise floors.
* `xr: RUNTIME PERIOD CHANGED` - predictedDisplayPeriod transitions.
* `aimray` - the crosshair's ONE RAY (origin now `camera::render_pos`, not the
  retired `camera+0x80`).

### Next steps

1. **Phase 0 of `FLICKER_IMPLEMENTATION_PLAN.md`** - the engine view boundary
   and an identity that survives to the palette draw. Everything else waits.
2. VR-57 the crosshair: step 1 done (`aim_ray.cpp`); steps 2-6 need the dot.
3. VR-67 performance: the 57/s window's split, not a resolution change.
4. VR-64 weapon swap flicker: `wa/key:` read once - `bolt_01` re-matched on
...
`GamepadOnly=0`, 90 Hz. Backups: `dishonored_vr.ini.bak-preVR57`,
`.bak-preEyeOffset`.
...
### 2026-09-09 - the flicker session: six fixes, eight dead ends, four broken instruments

Four instruments produced confident numbers about the wrong population, and the
pattern is worth more than any of them:

* an audit whose two inputs both came from one pose consume - could only print zero;
* a publication stamp that measured the producer when the question was the consumer;
* a hold whose fault never fired while the symptom continued;
* a probe that compared one draw context with ITSELF, because the caller shares a
  context across two hand ranges - 105,816 "pairs" with 0.00 variance.

**Verify the POPULATION before reading the number**, and print it on the line.
The fifth instrument does, and it is the only one that produced a usable answer.

Three more lessons, each paid for:

* **Ask what ELSE would move a counter the same way.** Agreement went 0.2 % to
  97.9 % on a sign flip; in an alternating stream "one publication late" and
  "opposite convention" give identical counts.
* **A stale comment in a neighbouring file is not a source.** `camera+0x80` was
  used as a camera position; ENGINE_NOTES had measured it as a fixed offset
  vector and retired it, and the instrument printed the retired constant on its
  own first line.
* **An experiment left armed contaminates every measurement after it.**
  `PoseLagAb` ran 0/2/0/2 for four runs and the judder "came back".
