...
## CURRENT (2026-09-13): load/reload fixes confirmed; branch approved for merge
...
The maintainer authorized commit, PR and merge, and the complete installed INI
and saved F10 profile as repo defaults. The packaged ini is its byte copy; the
...
branch closure. No game was launched by the agent and no release is declared.
