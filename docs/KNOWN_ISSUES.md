...
- **The sword's swing trail is hidden** [VR-171]. The game's swoosh follows its own attack
  animation, not the blade in your hand, so it is not drawn for the player's swings (enemy
...
- **The HUD is back on a window and on the hands, per element** [S3, VR-117, VR-120]. Every
  ...
  and mana together: they interleave), the reticle and the interaction prompt are named by
  a measured rectangle; equipment, subtitles, the objective marker, toasts, tutorials, the
  detection arrows, the skip gauge and dark vision ride `default` (the window) until a
  rectangle is measured for them (`draws on` + `draws regions`, then `hud region <name>
  x0,y0,x1,y1` live). A screen set to off or frame takes the mono screen. Dark HUD strokes
  read faint on the quads with the shipped `AlphaMode=repair`; `captured` keeps them solid
  and waits for the headset's verdict (VR-119). `hud off` puts the HUD back in the frame.
