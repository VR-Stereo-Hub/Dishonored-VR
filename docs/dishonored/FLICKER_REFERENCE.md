...
| Grass (and some other objects) invisible for one or two frames while walking in a straight line | OPEN, suspects: (a) the VR-79 `pereye` right-eye view state (its own occlusion history; a stale occluded result on a component re-attached as you walk), (b) older than VR-79: the early report of grass and objects vanishing up close | VR-226 REPORTED 2026-09-24 with `Occlusion=pereye`; not measured. Discriminators: same walk under `occlusion native` (still blinks = not pereye), then one eye closed at a time (right eye only = the second view state) |
