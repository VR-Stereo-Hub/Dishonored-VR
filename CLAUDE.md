...
- `src/tools/` - `xrsim/` (the simulated OpenXR runtime), `ovrshim/` (the SteamVR shim),
  `installer/` (DishonoredVR-Launcher.exe, the player's installer in the F10 theme),
...
| `docs/dishonored/HUD_ANCHORS.md` | **The HUD on its anchors** (VR-117..VR-120): the redirect, the element table, the six anchors, the alpha modes, every key and word, and every measurement in section 7 |
| `docs/dishonored/HUD_ELEMENTS_HOWTO.md` | **How to measure and add a HUD element**: find its rectangle in the log, name it live, bake it into the table and the ini, what a rectangle cannot claim |
| `docs/dishonored/PHYSICAL_SWING.md` | **The motion sword** (VR-37): the edge and sustain detectors, the sample feed and why it keys on the hand generation, the gates, the attack pulse and the honoured-check, every `[Melee]` key and `swing` word, the headset tuning guide, the graveyard. Section 2b is the hump census (VR-170): how a threshold is set from a log |
| `docs/dishonored/PLAN-contact-sword.md` | **The contact-timed sword** (VR-173, research, not built): what the sibling mod's "servo" really is, prior art and its numbers, the five prerequisites in order, and a brief to paste into the session that does it |
